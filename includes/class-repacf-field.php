...
if ( ! class_exists( 'acf_field_repeater' ) && class_exists('acf_field') ) :
	...
	class acf_field_repeater extends acf_field {
		...
		*/
		...
		function initialize() {
			...
			// vars
			$this->name     = 'repeater';
			$this->label    = __( 'Repeater (Repeater field for ACF)', 'repeater-acf' );
		...
		*/
		...
		function input_admin_enqueue_scripts() {
			...
			acf_localize_text(
				array(
					'Minimum rows reached ({min} rows)' => __( 'Minimum rows reached ({min} rows)', 'repeater-acf' ),
					'Maximum rows reached ({max} rows)' => __( 'Maximum rows reached ({max} rows)', 'repeater-acf' ),
				)
			);
		}
		...
		*/
		...
		function render_field( $field ) {
			...
			if ( $field['button_label'] === '' ) {
				$field['button_label'] = __( 'Add Row', 'repeater-acf' );
			}
...
<table class="acf-table">
	...
	<tbody>
			...
			" data-id="<?php echo esc_attr( $id ); ?>">
				...
				<?php if ( $show_order ) : ?>
					<td class="acf-row-handle order" title="<?php _e( 'Drag to reorder', 'repeater-acf' ); ?>">
						<?php if ( $field['collapsed'] ) : ?>
						<a class="acf-icon -collapse small" href="#" data-event="collapse-row" title="<?php _e( 'Click to toggle', 'repeater-acf' ); ?>"></a>
						<?php endif; ?>
				...
				<?php endforeach; ?>
					...
					<td class="acf-row-handle remove">
						<a class="acf-icon -plus small acf-js-tooltip hide-on-shift" href="#" data-event="add-row" title="<?php _e( 'Add row', 'repeater-acf' ); ?>"></a>
						<a class="acf-icon -duplicate small acf-js-tooltip show-on-shift" href="#" data-event="duplicate-row" title="<?php _e( 'Duplicate row', 'repeater-acf' ); ?>"></a>
						<a class="acf-icon -minus small acf-js-tooltip" href="#" data-event="remove-row" title="<?php _e( 'Remove row', 'repeater-acf' ); ?>"></a>
					</td>
				<?php endif; ?>
	...
	<div class="acf-actions">
		...
		*/
		...
		<tr class="acf-field acf-field-setting-sub_fields" data-setting="repeater" data-name="sub_fields">
			<td class="acf-label">
				<label><?php _e( 'Sub Fields', 'repeater-acf' ); ?></label>
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Collapsed', 'repeater-acf' ),
					'instructions' => __( 'Select a sub field to show when row is collapsed', 'repeater-acf' ),
					'type'         => 'select',
					'name'         => 'collapsed',
					'allow_null'   => 1,
					'choices'      => $choices,
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Minimum Rows', 'repeater-acf' ),
					'instructions' => '',
					'type'         => 'number',
					'name'         => 'min',
					'placeholder'  => '0',
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Maximum Rows', 'repeater-acf' ),
					'instructions' => '',
					'type'         => 'number',
					'name'         => 'max',
					'placeholder'  => '0',
			...
			acf_render_field_setting(
				...
				array(
					...
					'layout'       => 'horizontal',
					'choices'      => array(
						'table' => __( 'Table', 'repeater-acf' ),
						'block' => __( 'Block', 'repeater-acf' ),
						'row'   => __( 'Row', 'repeater-acf' ),
					),
				)
			);
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Button Label', 'repeater-acf' ),
					'instructions' => '',
					'type'         => 'text',
					'name'         => 'button_label',
					'placeholder'  => __( 'Add Row', 'repeater-acf' ),
				)
			);
		...
		*/
		...
		function validate_value( $valid, $value, $field, $input ) {
			...
			if ( $min && $count < $min ) {
				...
				// create error
				$error = __( 'Minimum rows reached ({min} rows)', 'repeater-acf' );
		...
		*/
		...
		public function validate_rest_value( $valid, $value, $field ) {
			if ( ! is_array( $value ) && is_null( $value ) ) {
				...
				$error = sprintf( __( '%s must be of type array or null.', 'repeater-acf' ), $param );
